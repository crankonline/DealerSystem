...
<script type="text/javascript">
    ...
    RequisitesForm
        ...
        .controller('RequisitesFormData', ['$scope', '$http', 'mObjNode', 'Upload', '$timeout', '$sce', '$window', function ($scope, $http, mObjNode, Upload, $timeout, $sce, $window) {
            ...
            $scope.SM = '';
            $scope.toggle = true;
            $http.post('<?php echo base_url("index.php/requisites/reference_load"); ?>', {
            ...
            angular.forEach($scope.Data.common.files, function (item) { //грузим сканы юрлица
                //console.log(item);
                $http.post('<?php echo base_url(); ?>index.php/requisites/get_image_reference', {
            ...
            $scope.Get_person = function (Series, Number, Key) {
                if ((Series && Series.length > 1) && (Number && Number.length > 5)) {
                    $http.post('<?php echo base_url(); ?>index.php/requisites/get_person_by_passport_reference', {
                        series: Series,
                        number: Number
                    }).then(function (response) {
                    ...
                    if ($scope.mu_file_kg) {
                        Upload.upload({
                            ...
                            if (responsejur.status > 0) {
                                $scope.ErrorFunc('<p>Ошибка при сохранении изображений юридического лица, код ошибки: ' + responsejur.status + '.</p><p> Сообщение: ' + responsejur.data + '</p>');
                            }
                        }, function (evt) {
                        ...
                        if ($scope.passport_side_1[i]) {
                            Upload.upload({
                                ...
                                if (responsephy.status > 0) {
                                    $scope.ErrorFunc('<p>Ошибка при сохранении изображений ответственных лиц, код ошибки: ' + responsephy.status + '.</p><p> Сообщение: ' + responsephy.data + '</p>');
                                }
                            }, function (evt) {
