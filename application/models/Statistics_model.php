...
(
  SELECT
    ...
    CASE WHEN inventory_id = 1 OR inventory_id = 3 OR inventory_id = 5 OR inventory_id = 6 THEN "count" ELSE \'0\' END as edscount,
...
ORDER BY requisites_creating_date_time DESC';
    ...
    public function get_statistics_error_eds_pki_ext($period_start, $period_end, $user_id = null) {
        $this->db->select('invoice.invoice_serial_number')->
                select('invoice.inn')->
                select('COALESCE((SELECT "count" FROM "Dealer_data".sell WHERE invoice_id=id_invoice AND (inventory_id =1 OR inventory_id =3 OR inventory_id =5 OR inventory_id=6)),\'0\') AS EDSCount')->//???????
